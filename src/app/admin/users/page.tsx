...
export default function ManageUsersPage() {
  ...
  const handleUpdateCoins = async () => {
    ...
    try {
      ...
      // Atomic increment - async non-blocking
      updateDoc(userRef, {
        coins: increment(amount)
      }).catch((e) => console.error("Update coins error", e));
      ...
      }).catch((e) => console.error("Log transaction error", e));
      ...
      // SUCCESS FLOW: REGULATED CLOSING SEQUENCE
      // 1. Close modal trigger immediately
      setCoinAction(null)
      
      // 2. Small delay to allow Radix UI to clean up DOM and transitions
      // This prevents the "Frozen UI" bug during background Firestore re-renders
      setTimeout(() => {
        setSelectedUser(null)
        setCoinAmount(0)
        setIsProcessing(false)
        
        // 3. Fail-safe forced cleanup
  ...
  return (
    <div className="space-y-6">
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1)
                  ...
                  .map((p, idx, arr) => (
                    <React.Fragment key={p}>
                      {idx > 0 && arr[idx-1] !== p - 1 && <span className="px-2 text-muted-foreground self-end">...</span>}
                      <Button
                        variant={currentPage === p ? "default" : "outline"}
                        onClick={() => setCurrentPage(p)}
                        className={cn(
                          "h-10 w-10 rounded-xl font-bold transition-all",
                          currentPage === p ? "bg-primary text-white" : "border-white/10 bg-transparent text-muted-foreground"
                        )}
                      >
                        {p}
                      </Button>
      ...
      <Dialog 
        open={!!coinAction} 
        onOpenChange={(open) => {
          // Block closure during processing to prevent cleanup issues
      ...
      >
        <DialogContent className="bg-black/95 border-white/10 text-white rounded-[2rem]">
          ...
          <DialogFooter>
            ...
            >
              {isProcessing ? <Loader2 className="animate-spin h-5 w-5" /> : "Confirm Update"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
