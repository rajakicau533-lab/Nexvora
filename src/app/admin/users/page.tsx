...
export default function ManageUsersPage() {
  ...
  const filteredUsers = React.useMemo(() => {
    ...
    return users.filter(u => {
      ...
      // Filter based on adminVerified status as requested
      const matchesVerify = statusFilter === 'all' || 
                           (statusFilter === 'verified' && u.adminVerified === true) ||
                           (statusFilter === 'unverified' && !u.adminVerified);
  ...
  const handleUpdateCoins = async () => {
    ...
    try {
      ...
      // Safety closure to prevent UI freeze
      setCoinAction(null);
      setTimeout(() => {
  ...
  const handleDeleteUser = async (user: any) => {
    ...
    if (!confirm(`Yakin ingin menghapus user "${user.username}" secara permanen dari database?`)) return
    ...
    try {
      ...
      await addDoc(collection(db, "activity_logs"), {
        ...
        userId: currentUser?.uid,
        userEmail: currentUser?.email,
        details: `Deleted user ${user.email} (${user.uid})`,
        timestamp: serverTimestamp()
      });

      toast({ title: "User Dihapus", description: "Data telah dihapus secara permanen dari database." });
    } catch (err: any) {
