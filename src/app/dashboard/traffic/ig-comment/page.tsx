...
import { Clock, ExternalLink, Loader2, Info, AlertCircle, RefreshCw, Send, History, FileText, Instagram } from "lucide-react"
...
const TEMPLATE_COMMENTS = [
  "Cek DM kak",
  "Harga berapa ya kak?",
  "OOTD-nya keren banget!",
  "Beli dimana kak produknya?",
  "Review-nya sangat membantu",
  "Semangat terus kak kontennya",
  "Bagus banget fotonya",
  "Inspiratif sekali postingannya",
  "Mau dong info lengkapnya",
  "Mantap, sukses selalu kak"
];
...
export default function InstagramCommentPage() {
  ...
  const serviceConfig = TRAFFIC_SERVICES.IG_COMMENT
  ...
  // Perhitungan: 2 komentar = 1 koin
  const coinCost = Math.ceil(quantity / 2);
  const isValidQuantity = quantity >= 4 && quantity <= 100;
  ...
  const handleOrder = async () => {
    ...
    if (!url) { toast({ variant: "destructive", title: "Link Wajib", description: "Masukkan link Instagram." }); return; }
    if (!isValidQuantity) { toast({ variant: "destructive", title: "Jumlah Salah", description: "Minimal 4 komentar." }); return; }
    ...
    try {
      ...
      toast({ title: "Pesanan Sukses! 🚀", description: "Komentar Instagram sedang diproses." });
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-1">
          <h2 className="text-3xl font-headline font-bold text-white flex items-center gap-3">
             <Instagram className="text-pink-500 h-8 w-8" /> Komentar IG 💬
          </h2>
          <p className="text-muted-foreground text-sm">Tingkatkan engagement postingan Instagram dengan komentar kustom.</p>
        </div>
        ...
        <div className="lg:col-span-8">
          <Card className="premium-card rounded-[2rem] border-white/5 bg-black/40 shadow-2xl overflow-hidden">
            <CardHeader className="p-8 pb-4">
               <CardTitle className="text-xl text-white">Order Komentar Instagram</CardTitle>
               <CardDescription>2 Komentar = 1 Koin. Minimal 4 komentar.</CardDescription>
            </CardHeader>
            <CardContent className="p-8 pt-6 space-y-6">
              <div className="space-y-2">
                <Label className="text-xs font-black uppercase text-white/40 tracking-widest ml-1">Link Postingan/Reels Instagram</Label>
                <Input 
              ...
              <div className="space-y-3">
                ...
                <Textarea 
                  placeholder={"Contoh:\nWah bagus banget kak produknya\nCek DM ya kak\nRecommended banget"}
                  value={commentsText}
                ...
                <div className="flex justify-between text-[10px] font-black uppercase text-white/30 px-1">
                  <span className={cn(quantity < 4 && quantity > 0 ? "text-red-500 font-bold" : "text-white/30")}>
                    Terhitung: {quantity} Komentar {quantity < 4 && quantity > 0 && "(Min 4)"}
                  </span>
        ...
        <div className="lg:col-span-4 space-y-6">
          <Card className="premium-card rounded-3xl border-white/5 bg-black/40 h-fit">
            ...
            <CardContent className="p-6 text-[11px] space-y-4 text-muted-foreground leading-relaxed">
              <div className="space-y-2">
                <p>• Tarif: <strong>2 Komentar = 1 Koin</strong>.</p>
                <p>• Minimal: <strong>4 komentar</strong> per order.</p>
                <p>• Akun/Konten tidak boleh dalam mode <strong>Privat</strong>.</p>
              </div>
              <div className="p-4 rounded-2xl bg-primary/5 border border-primary/10 flex gap-3">
                 <AlertCircle className="h-4 w-4 text-primary shrink-0 mt-0.5" />
                 <p className="text-[10px] leading-snug">Sangat efektif untuk meningkatkan rating algoritma pada konten Reels baru.</p>
              </div>
            </CardContent>
          </Card>
        </div>
