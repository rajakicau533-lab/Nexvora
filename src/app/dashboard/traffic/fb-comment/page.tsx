...
import { Clock, ExternalLink, Loader2, Info, AlertCircle, RefreshCw, Send, History, FileText, Facebook } from "lucide-react"
...
const TEMPLATE_COMMENTS = [
  ...
  "Semoga lancar terus usahanya",
  "Mantap nih rekomendasinya",
  "Fix mau coba juga",
  "Izin share ya kak",
  "Keren abis pokoknya",
  "Ditunggu update selanjutnya",
  "Produknya bermanfaat banget"
];
...
export default function FacebookCommentPage() {
  ...
  const serviceConfig = TRAFFIC_SERVICES.FB_COMMENT
  ...
  const handleOrder = async () => {
    ...
    if (!url) { toast({ variant: "destructive", title: "Link Wajib", description: "Masukkan link postingan Facebook." }); return; }
    ...
    try {
      ...
      toast({ title: "Pesanan Sukses! 🚀", description: "Komentar Facebook sedang diproses." });
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-1">
          <h2 className="text-3xl font-headline font-bold text-white flex items-center gap-3">
            <Facebook className="text-blue-500 h-8 w-8" /> Komentar FB 💬
          </h2>
          <p className="text-muted-foreground text-sm">Meningkatkan interaksi postingan Facebook dengan komentar kustom.</p>
        </div>
        ...
        <div className="lg:col-span-8">
          <Card className="premium-card rounded-[2rem] border-white/5 bg-black/40 shadow-2xl overflow-hidden">
            <CardHeader className="p-8 pb-4">
               <CardTitle className="text-xl text-white">Order Komentar Facebook</CardTitle>
               <CardDescription>2 Komentar = 1 Koin. Minimal 4 komentar.</CardDescription>
            </CardHeader>
            <CardContent className="p-8 pt-6 space-y-6">
              <div className="space-y-2">
                <Label className="text-xs font-black uppercase text-white/40 tracking-widest ml-1">Link Postingan Facebook</Label>
                <Input 
          ...
          <Card className="premium-card rounded-3xl border-white/5 bg-black/40 h-fit">
            ...
            <CardContent className="p-6 text-[11px] space-y-4 text-muted-foreground leading-relaxed">
              <div className="space-y-2">
                ...
                <p>• Pastikan akun/postingan bersifat <strong>Publik</strong>.</p>
              </div>
              <div className="p-4 rounded-2xl bg-primary/5 border border-primary/10 flex gap-3">
                 <AlertCircle className="h-4 w-4 text-primary shrink-0 mt-0.5" />
                 <p className="text-[10px] leading-snug">Gunakan akun asli untuk menghindari deteksi spam oleh sistem Facebook.</p>
              </div>
            </CardContent>
          </Card>
        </div>
